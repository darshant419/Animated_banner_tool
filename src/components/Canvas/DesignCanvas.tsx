...
const BoardStage: React.FC<BoardStageProps> = ({ board, isActive, registerStage }) => {
  ...
  // Keyboard nudging G�� only the active board handles it (others are mounted too).
  useEffect(() => {
    if (!isActive) return;
  ...
  return (
    ...
    >
      ...
      {/* On-canvas ISI rich-content editor G�� rendered via a portal so the banner's
...
export const DesignCanvas: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const handleGetThumbnail = (e: any) => {
      const stage = activeArtboardId ? stageRegistry.current.get(activeArtboardId) : null;
  ...
  return (
    <div className="flex-1 bg-gray-100 flex flex-col items-center justify-start overflow-auto p-8 pt-16 relative">
      ...
      {multiArtboardView ? (
        /* Multi-size view G�� every artboard live & editable side by side */
